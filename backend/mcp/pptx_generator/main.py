...
if main_log_path.exists():
    ...
    file_handler.setFormatter(
        logging.Formatter(
            "%(asctime)s - PPTX_GENERATOR - %(name)s - %(levelname)s - %(message)s"
        )
    )
...
def json_to_pptx(
    input_data: Annotated[
        str,
        'JSON string containing slide data in this format: {"slides": [{"title": "Slide 1", "content": "- Item 1\\n- Item 2\\n- Item 3"}, {"title": "Slide 2", "content": "- Item A\\n- Item B"}]}',
    ],
    output_filename: Annotated[
        Optional[str], "Base name for output files (without extension)"
    ] = "presentation",
    image_filename: Annotated[
        Optional[str], "Optional image filename to integrate into the presentation"
    ] = "",
    image_data_base64: Annotated[
        Optional[str], "Framework may supply Base64 image content as fallback"
    ] = "",
) -> Dict[str, Any]:
    """
    ...
    **JSON Format Example:**
    ...
    try:
        ...
        if not isinstance(data, dict) or "slides" not in data:
            return {
                "results": {
                    "error": "Input must be a JSON object containing 'slides' array"
                }
            }
        ...
        <div class="slide-content">"""
                ...
                if i == 0 and image_bytes:
                    try:
                        ...
                        if VERBOSE:
                            logger.warning(
                                f"Could not process image for HTML conversion: {e}"
                            )
            ...
            if os.path.exists(html_output_path):
                ...
                html_b64 = base64.b64encode(html_content_file.encode("utf-8")).decode(
                    "utf-8"
                )
                if VERBOSE:
            ...
            return {
                ...
                "meta_data": {
                    ...
                    "output_files": [
                        f"{output_filename}.pptx",
                        f"{output_filename}.html",
                    ]
                    if html_b64
                    else [f"{output_filename}.pptx"],
                    "output_file_paths": [
                        f"temp:output_{output_filename}.pptx",
                        f"temp:output_{output_filename}.html",
                    ]
                    if html_b64
                    else [f"temp:output_{output_filename}.pptx"],
                },
            }
    except Exception as e:
        if VERBOSE:
...
def markdown_to_pptx(
    markdown_content: Annotated[
        str,
        "Markdown content with headers (# or ##) as slide titles and content below each header",
    ],
    output_filename: Annotated[
        Optional[str], "Base name for output files (without extension)"
    ] = "presentation",
    ...
    try:
        ...
        if not slides:
            return {
                "results": {"error": "No slides could be parsed from markdown content"}
            }
...
<body>"""
    ...
    <div class="slide">
        ...
        return {
            "results": {"error": f"Error creating PowerPoint from markdown: {str(e)}"}
        }
