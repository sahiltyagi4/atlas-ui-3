...
def execute_python_code_with_file(
    ...
    filename: Annotated[
        str,
        "Name of the file to make available to the code (optional - leave empty if not uploading a file)",
    ] = "",
    ...
    try:
        ...
        if filename:
            try:
                ...
                if VERBOSE:
                    logger.info(
                        f"Using clean filename: {clean_filename} (from original: {filename})"
                    )
                saved_filename = save_file_to_execution_dir(
                    clean_filename, file_data_base64, exec_dir
                )
                if VERBOSE:
                    logger.info(
                        f"Saved file as: {saved_filename} in directory: {exec_dir}"
                    )
        ...
        if execution_result.get("success"):
            ...
            if plots or raw_output.strip():
                try:
                    ...
                    html_content_b64 = base64.b64encode(
                        visualization_html.encode("utf-8")
                    ).decode("utf-8")
            ...
            results_payload = {"error": error_msg, "stdout": truncated_output}
            if execution_result.get("stderr"):
                stderr_val = execution_result.get("stderr", "")
                if len(stderr_val) > 800:
                    results_payload["stderr"] = stderr_val[:800] + "... [truncated]"
