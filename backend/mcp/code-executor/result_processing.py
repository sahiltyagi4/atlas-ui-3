            ...
            if file_path.is_file() and file_path.name != "exec_script.py":
                try:
                    ...
                    encoded_files.append(
                        {"filename": file_path.name, "content_base64": content_base64}
                    )
                    logger.info(
                        f"Encoded file: {file_path.name} ({len(file_content)} bytes)"
                    )
                except Exception as e:
...
def truncate_output_for_llm(output: str, max_chars: int = 2000) -> tuple[str, bool]:
    ...
    if important_sections:
        ...
        for start, end, term in important_sections:
            if (
                merged_sections and start <= merged_sections[-1][1] + 50
            ):  # Merge if close
                merged_sections[-1] = (
                    merged_sections[-1][0],
                    max(end, merged_sections[-1][1]),
                    merged_sections[-1][2] + f", {term}",
                )
            else:
        ...
        for start, end, terms in merged_sections:
            ...
            if (
                total_chars + len(section) + 100 > max_chars
