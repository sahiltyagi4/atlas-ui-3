...
async def require_admin_for_feedback(
...
async def submit_feedback(
    ...
    try:
        ...
        logger.info(
            f"Feedback submitted by {sanitize_for_logging(current_user)}: rating={feedback.rating}, file={sanitize_for_logging(filename)}"
        )
...
async def get_feedback_stats(
    ...
    try:
                ...
                with open(feedback_file, "r", encoding="utf-8") as f:
                    ...
                    if "timestamp" in feedback_data:
                        feedback_time = datetime.fromisoformat(
                            feedback_data["timestamp"].replace("Z", "+00:00")
                        )
        ...
        return {
            ...
            "feedback_with_comments": sum(
                1 for fb in all_feedback if fb.get("comment", "").strip()
            ),
        ...
        raise HTTPException(
            status_code=500, detail="Failed to calculate feedback statistics"
        )


@feedback_router.delete("/feedback/{feedback_id}")
async def delete_feedback(
    feedback_id: str, admin_user: str = Depends(require_admin_for_feedback)
):
    ...
    try:
        ...
        logger.info(
            f"Feedback {sanitize_for_logging(feedback_id)} deleted by {sanitize_for_logging(admin_user)}"
        )
