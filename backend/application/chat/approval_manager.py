...
class ToolApprovalManager:
    ...
    def create_approval_request(
        ...
        logger.info(
            f"Created approval request for tool {sanitize_for_logging(tool_name)} (call_id: {sanitize_for_logging(tool_call_id)})"
        )
        return request
    ...
    def handle_approval_response(
        ...
        """
        logger.info(
            f"handle_approval_response called: tool_call_id={sanitize_for_logging(tool_call_id)}, approved={sanitize_for_logging(approved)}"
        )
        logger.info(
            f"Pending requests: {[sanitize_for_logging(key) for key in self._pending_requests.keys()]}"
        )

        request = self._pending_requests.get(tool_call_id)
        if request is None:
            logger.warning(
                f"Received approval response for unknown tool call: {sanitize_for_logging(tool_call_id)}"
            )
            logger.warning(
                f"Available pending requests: {list(self._pending_requests.keys())}"
            )
            return False

        logger.info(
            f"Found pending request for {sanitize_for_logging(tool_call_id)}, setting response"
        )
        request.set_response(approved, arguments, reason)
        # Keep the request in the dict for a bit to avoid race conditions
        # It will be cleaned up later
        logger.info(
            f"Approval response handled for tool {sanitize_for_logging(request.tool_name)}: approved={approved}"
        )
        return True
