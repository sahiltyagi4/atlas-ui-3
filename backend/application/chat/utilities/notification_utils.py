            ...
            for k, v in obj.items():
                if k == "filename":
                    ...
                    if "filename" in inner:
                        inner["filename"] = _sanitize_filename_value(
                            inner.get("filename")
                        )
...
async def notify_tool_start(
    tool_call, parsed_args: Dict[str, Any], update_callback: Optional[UpdateCallback]
) -> None:
    """
...
async def _handle_structured_progress_update(
    ...
    if update_type == "canvas_update":
        ...
        if content:
            await safe_notify(
                update_callback, {"type": "canvas_content", "content": content}
            )
        ...
        if artifacts:
            ...
            logger.info(
                f"Tool {tool_name} sent {len(artifacts)} artifact(s) during execution"
            )
...
async def notify_canvas_content(
    parsed_args: Dict[str, Any], update_callback: UpdateCallback
) -> None:
    """
    ...
    try:
        content_arg = (
            parsed_args.get("content") if isinstance(parsed_args, dict) else None
        )
        if content_arg:
            logger.info(
                "Emitting canvas_content event (length=%s)",
                len(content_arg) if isinstance(content_arg, str) else "obj",
            )
            await safe_notify(
                update_callback, {"type": "canvas_content", "content": content_arg}
            )
        else:
            logger.info(
                "Canvas tool called without 'content' arg; skipping canvas_content event"
            )
    except Exception as e:
...
async def notify_tool_error(
    tool_call, error: str, update_callback: Optional[UpdateCallback]
) -> None:
    """
...
async def notify_tool_synthesis(
    message: str, update_callback: Optional[UpdateCallback]
) -> None:
    """
    ...
    if message and message.strip():
        await safe_notify(
            update_callback, {"type": "tool_synthesis", "message": message}
        )


async def notify_agent_update(update_type: str, connection, **kwargs) -> None:
    """
    ...
    try:
        payload = {"type": "agent_update", "update_type": update_type, **kwargs}
...
async def notify_files_update(
    organized_files: Dict[str, Any], update_callback: Optional[UpdateCallback]
) -> None:
    """
...
async def notify_canvas_files(
    canvas_files: List[Dict[str, Any]], update_callback: Optional[UpdateCallback]
) -> None:
    """
...
def create_error_response(
    error_message: str, message_type: str = "error"
) -> Dict[str, str]:
    """
    ...
    """
    return {"type": message_type, "message": error_message}


def create_chat_response(
    message: str, message_type: str = "chat_response"
) -> Dict[str, str]:
    """
