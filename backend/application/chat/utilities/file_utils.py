...
async def ingest_tool_files(
    ...
    contents = (
        tool_result.returned_file_contents[:MAX_FILES]
        if tool_result.returned_file_contents
        else []
    )
    ...
    try:
        ...
        for artifact in artifacts:
            ...
            files_to_upload.append(
                {"filename": name, "content": b64_content, "mime_type": mime_type}
            )
        ...
        if uploaded_refs and artifact_names:
            ...
            for fname in artifact_names:
                ...
                if meta and file_manager.should_display_in_canvas(fname):
                    ...
                    artifact = next(
                        (a for a in tool_result.artifacts if a.get("name") == fname), {}
                    )
                    mime_type = artifact.get("mime")
            ...
            if canvas_files:
                ...
                primary = None
                if tool_result.display_config and isinstance(
                    tool_result.display_config, dict
                ):
