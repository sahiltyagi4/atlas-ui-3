...
async def safe_get_tools_schema(
    tool_manager, selected_tools: List[str]
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        logger.info(
            f"Got {len(tools_schema)} tool schemas for selected tools: {selected_tools}"
        )
        return tools_schema
    except Exception as e:
...
async def safe_call_llm_with_tools(
    ...
    try:
        if data_sources and user_email:
            ...
            logger.info(
                f"LLM response received with RAG and tools for user {user_email}, has_tool_calls: {llm_response.has_tool_calls()}"
            )
        else:
            ...
            logger.info(
                f"LLM response received with tools only, llm_response: {llm_response}"
            )
        return llm_response
    except Exception as e:
...
async def safe_file_operation(file_operation_func: Callable, *args, **kwargs) -> Any:
    """
...
async def safe_llm_call(llm_call_func: Callable, *args, **kwargs) -> Any:
    """
...
def safe_sync_operation(operation_func: Callable, *args, **kwargs) -> Any:
    """
...
def create_error_response(
    ...
    """
    return {"type": error_type, "message": str(error_message)}


def create_validation_error_response(validation_message: str) -> Dict[str, str]:
    """
...
def log_and_suppress_error(
    operation_name: str, error: Exception, level: str = "warning"
) -> None:
    """
...
def handle_chat_message_error(
    error: Exception, context: str = "chat message handling"
) -> Dict[str, str]:
    """
    ...
    return {"type": MessageType.ERROR.value, "message": str(error)}
...
async def with_retry(
    operation_func: Callable, max_retries: int = 3, *args, **kwargs
) -> Any:
    """
