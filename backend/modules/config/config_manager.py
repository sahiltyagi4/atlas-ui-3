...
class MCPServerConfig(BaseModel):
    ...
    transport: Optional[str] = (
        None  # Explicit transport: "stdio", "http", "sse" - takes priority over auto-detection
    )
    auth_token: Optional[str] = None  # Bearer token for MCP server authentication
    compliance_level: Optional[str] = (
        None  # Compliance/security level (e.g., "SOC2", "HIPAA", "Public")
    )
    require_approval: List[str] = Field(
        default_factory=list
    )  # List of tool names (without server prefix) requiring approval
    allow_edit: List[str] = Field(
        default_factory=list
    )  # LEGACY. List of tool names (without server prefix) allowing argument editing
...
class MCPConfig(BaseModel):
    ...
    def validate_servers(cls, v):
        ...
        if isinstance(v, dict):
            return {
                name: MCPServerConfig(**config) if isinstance(config, dict) else config
                for name, config in v.items()
            }
        return v
...
class ToolApprovalsConfig(BaseModel):
    ...
    def validate_tools(cls, v):
        ...
        if isinstance(v, dict):
            return {
                name: ToolApprovalConfig(**config)
...
class AppSettings(BaseSettings):
    ...
    # Logging settings
    log_level: str = (
        "INFO"  # Override default logging level (DEBUG, INFO, WARNING, ERROR)
    )
    ...
    feature_splash_screen_enabled: bool = Field(
        ...
        validation_alias=AliasChoices(
            "FEATURE_SPLASH_SCREEN_ENABLED", "SPLASH_SCREEN_ENABLED"
        ),
    )
    ...
    feature_agent_mode_available: bool = Field(
        ...
        validation_alias=AliasChoices(
            "FEATURE_AGENT_MODE_AVAILABLE", "AGENT_MODE_AVAILABLE"
        ),
    ...
    force_tool_approval_globally: bool = Field(
        default=False, validation_alias="FORCE_TOOL_APPROVAL_GLOBALLY"
    )
    ...
    auth_group_check_url: Optional[str] = Field(
        default=None, validation_alias="AUTH_GROUP_CHECK_URL"
    )
    auth_group_check_api_key: Optional[str] = Field(
        default=None, validation_alias="AUTH_GROUP_CHECK_API_KEY"
    )
    ...
    prompt_base_path: str = (
        "prompts"  # Relative or absolute path to directory containing prompt templates
    )
    tool_synthesis_prompt_filename: str = (
        "tool_synthesis_prompt.md"  # Filename for tool synthesis prompt template
    )
    # Agent prompts
    agent_reason_prompt_filename: str = (
        "agent_reason_prompt.md"  # Filename for agent reason phase
    )
    agent_observe_prompt_filename: str = (
        "agent_observe_prompt.md"  # Filename for agent observe phase
    )
    ...
    rag_mcp_config_file: str = Field(
        default="mcp-rag.json", validation_alias="MCP_RAG_CONFIG_FILE"
    )
    llm_config_file: str = Field(
        default="llmconfig.yml", validation_alias="LLM_CONFIG_FILE"
    )
    help_config_file: str = Field(
        default="help-config.json", validation_alias="HELP_CONFIG_FILE"
    )
    messages_config_file: str = Field(
        default="messages.txt", validation_alias="MESSAGES_CONFIG_FILE"
    )
    tool_approvals_config_file: str = Field(
        default="tool-approvals.json", validation_alias="TOOL_APPROVALS_CONFIG_FILE"
    )
    splash_config_file: str = Field(
        default="splash-config.json", validation_alias="SPLASH_CONFIG_FILE"
    )

    # Config directory paths
    app_config_overrides: str = Field(
        default="config/overrides", validation_alias="APP_CONFIG_OVERRIDES"
    )
    app_config_defaults: str = Field(
        default="config/defaults", validation_alias="APP_CONFIG_DEFAULTS"
    )
    ...
    # Runtime directories
    runtime_feedback_dir: str = Field(
        default="runtime/feedback", validation_alias="RUNTIME_FEEDBACK_DIR"
    )
        ...
        if self._mcp_config is None:
            try:
                ...
                if data:
                    ...
                    logger.info(
                        f"Loaded MCP config with {len(self._mcp_config.servers)} servers: {list(self._mcp_config.servers.keys())}"
                    )
                else:
                    self._mcp_config = MCPConfig()
                    logger.info(
                        "Created empty MCP config (no configuration file found)"
                    )
        ...
        if self._rag_mcp_config is None:
            try:
                ...
                if data:
                    ...
                    # Validate compliance levels
                    self._validate_mcp_compliance_levels(
                        self._rag_mcp_config, "RAG MCP"
                    )
                    logger.info(
                    ...
                    logger.info(
                        "Created empty RAG MCP config (no configuration file found)"
                    )

            except Exception as e:
                logger.error(
                    f"Failed to parse RAG MCP configuration: {e}", exc_info=True
                )
                self._rag_mcp_config = MCPConfig()
        ...
        if self._tool_approvals_config is None:
            try:
                ...
                default_require_approval = (
                    self.app_settings.require_tool_approval_by_default
                )
                ...
                logger.info(
                    f"Built tool approvals config from mcp.json with {len(tools_config)} tool-specific settings (default: {default_require_approval})"
                )

            except Exception as e:
                logger.error(
                    f"Failed to build tool approvals configuration: {e}", exc_info=True
                )
                self._tool_approvals_config = ToolApprovalsConfig()
