...
class FileManager:
    ...
    async def upload_multiple_files(
        self, user_email: str, files: Dict[str, str], source_type: str = "user"
    ) -> Dict[str, str]:
    ...
    def organize_files_metadata(
        ...
        for filename, file_metadata in file_references.items():
            ...
            file_info = {
                ...
                "last_modified": file_metadata.get("last_modified").isoformat()
    ...
    async def upload_files_from_base64(
        self, files: List[Dict[str, Any]], user_email: str, source_type: str = "tool"
    ...
    def get_canvas_displayable_files(
        self, result_dict: Dict[str, Any], uploaded_files: Dict[str, str]
    ) -> List[Dict]:
        """Extract files from tool result that should be displayed in canvas."""
        canvas_files = []

        # Check returned_files array (preferred format)
        if "returned_files" in result_dict and isinstance(
            result_dict["returned_files"], list
        ):
            for file_info in result_dict["returned_files"]:
                if isinstance(file_info, dict) and "filename" in file_info:
                    ...
                    if (
                        self.should_display_in_canvas(filename)
                        and filename in uploaded_files
                    ):
                        canvas_files.append(
                            {
                                "filename": filename,
                                "type": self.get_canvas_file_type(
                                    self.get_file_extension(filename).lower()
                                ),
        ...
        # Check legacy single file format
            ...
            "returned_file_name" in result_dict
            and "returned_file_base64" in result_dict
        ):
        ...
        logger.info(
            f"Found {len(canvas_files)} canvas-displayable files: {[f['filename'] for f in canvas_files]}"
        )
        return canvas_files

    async def get_file_content(
        self, user_email: str, filename: str, s3_key: str
    ) -> Optional[str]:
