...
async def list_files(args) -> None:
    ...
    try:
        ...
        files = await s3_client.list_files(
            user_email=args.user_email, file_type=args.file_type, limit=args.limit
        )
        ...
        if not args.file_type:
            ...
            if user_files:
                ...
                for file_info in user_files:
                    ...
                    print(
                        f"      Modified: {file_info.get('last_modified', 'unknown')}"
                    )
                    print()
...
Examples:
  ...
  python -m backend.modules.file_storage.cli test-categorization example.py
    ...
    upload_parser.add_argument(
        "--filename", help="Custom filename (default: use original)"
    )
    upload_parser.add_argument(
        "--source-type", default="user", choices=["user", "tool"], help="Source type"
    )
    ...
    list_parser.add_argument(
        "--file-type", choices=["user", "tool"], help="Filter by file type"
    )
    list_parser.add_argument(
        "--limit", type=int, default=100, help="Maximum files to return"
    )
    ...
    download_parser.add_argument(
        "--output", "-o", help="Output filename (default: original filename)"
    )
    ...
    test_parser = subparsers.add_parser(
        "test-categorization", help="Test file categorization"
    )
