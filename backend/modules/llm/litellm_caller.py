...
class LiteLLMCaller:
    ...
    def _get_model_kwargs(
        self, model_name: str, temperature: Optional[float] = None
    ) -> Dict[str, Any]:
        ...
        if hasattr(model_config, "model_url") and model_config.model_url:
            if not any(
                provider in model_config.model_url
                for provider in ["openrouter", "api.openai.com", "api.anthropic.com"]
            ):
        ...
        if model_config.extra_headers:
            ...
            for header_key, header_value in model_config.extra_headers.items():
                try:
                    ...
                    logger.error(
                        f"Failed to resolve extra header '{header_key}' for model {model_name}: {e}"
                    )
                    raise
    ...
    async def call_plain(
        ...
        try:
            ...
            logger.info(
                f"Plain LLM call: {len(messages)} messages, {total_chars} chars"
            )

            response = await acompletion(
                model=litellm_model, messages=messages, **model_kwargs
            )

            content = response.choices[0].message.content or ""
            logger.info(
                f"LLM response preview: '{content[:200]}{'...' if len(content) > 200 else ''}'"
            )
            return content
    ...
    async def call_with_rag(
        ...
        try:
            ...
            rag_response = await rag_client.query_rag(user_email, data_source, messages)
            ...
            llm_response = await self.call_plain(
                model_name, messages_with_rag, temperature=temperature
            )
    ...
    async def call_with_tools(
        ...
        try:
            total_chars = sum(len(str(msg.get("content", ""))) for msg in messages)
            logger.info(
                f"LLM call with tools: {len(messages)} messages, {total_chars} chars, {len(tools_schema)} tools"
            )
            ...
            if tool_choice == "required" and not getattr(message, "tool_calls", None):
                logger.error(
                    f"LLM failed to return tool calls when tool_choice was 'required'. Full response: {response}"
                )
                raise ValueError(
                    "LLM failed to return tool calls when tool_choice was 'required'."
                )
            ...
            if tool_choice == "required" and final_tool_choice == "required":
                logger.warning(
                    f"Tool choice 'required' failed, retrying with 'auto': {exc}"
                )
                try:
                    response = await acompletion(
                        model=litellm_model,
                    ...
                    logger.error(
                        "Retry with tool_choice='auto' also failed: %s",
                        retry_exc,
                        exc_info=True,
                    )
    ...
    async def call_with_rag_and_tools(
        ...
        if not data_sources:
            return await self.call_with_tools(
                model_name, messages, tools_schema, tool_choice, temperature=temperature
            )
    ...
    def _format_rag_metadata(self, metadata) -> str:
        ...
        summary_parts.append(
            f" **Processing Time:** {metadata.query_processing_time_ms}ms"
        )

        if metadata.documents_found:
            summary_parts.append(
                f" **Documents Found:** {len(metadata.documents_found)} (searched {metadata.total_documents_searched})"
            )

            for i, doc in enumerate(metadata.documents_found[:3]):
                confidence_percent = int(doc.confidence_score * 100)
                summary_parts.append(
                    f"  • {doc.source} ({confidence_percent}% relevance, {doc.content_type})"
                )
