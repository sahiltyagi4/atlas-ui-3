...
class RAGClient:
    ...
    def _setup_test_client(self):
        ...
        try:
            ...
            rag_mock_path = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__), "..", "..", "..", "mocks", "rag-mock"
                )
            )
    ...
    async def discover_data_sources(self, user_name: str) -> List[DataSource]:
        ...
        if use_test_client:
            try:
                ...
                logger.error(
                    f"TestClient error while discovering data sources for {user_name}: {exc}",
                    exc_info=True,
                )
                return []
        ...
        # HTTP mode using unified client
            ...
            try:
                data = await self.http_client.get(
                    f"/v1/discover/datasources/{user_name}"
                )
                accessible_sources_data = data.get("accessible_data_sources", [])
            except HTTPException as exc:
                logger.warning(
                    f"HTTP error discovering data sources for {user_name}: {exc.detail}"
                )
                # Return empty list for graceful degradation instead of raising
                return []
            except Exception as exc:
                logger.error(
                    f"Unexpected error while discovering data sources for {user_name}: {exc}",
                    exc_info=True,
                )
                return []
    ...
    async def query_rag(
        ...
        if self.mock_mode and self.test_client:
            try:
                logger.info(
                    f"Using TestClient to query RAG for {sanitize_for_logging(user_name)} with data source {sanitize_for_logging(data_source)}"
                )
                ...
                logger.error(
                    f"TestClient error while querying RAG for {user_name}: {exc}",
                    exc_info=True,
                )
        ...
        try:
            data = await self.http_client.post(
                "/v1/chat/completions", json_data=payload
            )
            ...
            logger.error(
                f"Unexpected error while querying RAG for {user_name}: {exc}",
                exc_info=True,
            )
            raise HTTPException(status_code=500, detail="Internal server error")
