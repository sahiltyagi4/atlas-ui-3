...
def patch_mcp(monkeypatch):
                ...
                return types.SimpleNamespace(
                    structured_content={
                        "results": {
                            "hits": [
                                {
                                    "id": f"{self.server_name}-1",
                                    "score": 0.9,
                                    "resourceId": arguments.get("sources", [""])[0],
                                },
                                {
                                    "id": f"{self.server_name}-2",
                                    "score": 0.5,
                                    "resourceId": arguments.get("sources", [""])[-1],
                                },
                            ],
                ...
                return types.SimpleNamespace(
                    structured_content={
                        "results": {
                            ...
                            "citations": [
                                {"resourceId": r} for r in arguments.get("sources", [])
                            ],
                        }
                    }
                )
    ...
    async def fake_initialize_clients(self):
        ...
        self.clients = {
            "docsRag": FakeClient("docsRag"),
            "notionRag": FakeClient("notionRag"),
        }
    ...
    monkeypatch.setattr(
        MCPToolManager, "initialize_clients", fake_initialize_clients, raising=False
    )
    monkeypatch.setattr(
        MCPToolManager, "discover_tools", fake_discover_tools, raising=False
    )
    monkeypatch.setattr(
        MCPToolManager, "discover_prompts", fake_discover_prompts, raising=False
    )
    monkeypatch.setattr(
        MCPToolManager,
        "get_authorized_servers",
        fake_get_authorized_servers,
        raising=False,
    )
