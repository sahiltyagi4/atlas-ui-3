...
class TestMCPClientEnvironmentVariables:
    ...
    async def test_stdio_client_with_env_vars(
        self, mock_transport_class, mock_client_class, monkeypatch
    ):
        ...
        server_config = {
            ...
            "cwd": "backend",
            "env": {"VAR1": "literal-value", "VAR2": "another-literal"},
        }

        with patch(
            "backend.modules.mcp_tools.client.config_manager"
        ) as mock_config_manager:
            mock_config_manager.mcp_config.servers = {"test-server": Mock()}
            mock_config_manager.mcp_config.servers[
                "test-server"
            ].model_dump.return_value = server_config
    ...
    @patch("backend.modules.mcp_tools.client.Client")
    @patch("fastmcp.client.transports.StdioTransport")
    async def test_stdio_client_with_env_var_resolution(
        self, mock_transport_class, mock_client_class, monkeypatch
    ):
    ...
    @patch("backend.modules.mcp_tools.client.Client")
    @patch("fastmcp.client.transports.StdioTransport")
    async def test_stdio_client_without_env(
        self, mock_transport_class, mock_client_class
    ):
        """Should pass None when no env specified."""
        server_config = {"command": ["python", "server.py"], "cwd": "backend"}
    ...
    async def test_stdio_client_missing_env_var_fails(self, caplog):
        ...
        with patch(
            ...
            with patch("os.path.exists", return_value=True):
                ...
                result = await manager._initialize_single_client(
    ...
    @patch("backend.modules.mcp_tools.client.Client")
    @patch("fastmcp.client.transports.StdioTransport")
    async def test_stdio_client_with_env_no_cwd(
        self, mock_transport_class, mock_client_class, monkeypatch
    ):
    ...
    @patch("backend.modules.mcp_tools.client.Client")
    @patch("fastmcp.client.transports.StdioTransport")
    async def test_stdio_client_empty_env_dict(
        self, mock_transport_class, mock_client_class
    ):
        """Should handle empty env dict."""
        server_config = {"command": ["python", "server.py"], "env": {}}
